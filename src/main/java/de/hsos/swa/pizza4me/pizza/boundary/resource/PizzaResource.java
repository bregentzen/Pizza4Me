...
public class PizzaResource {
    ...
    @APIResponses({
            @APIResponse(responseCode = "200", description = "Successful operation",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = PizzaWebDTO.class))),
            @APIResponse(responseCode = "204", description = "No Content")
    })
    public Response getAllPizzas(@QueryParam("page") @DefaultValue("0") int page,
                                 @QueryParam("size") @DefaultValue("10") int size) {
        List<PizzaWebDTO> pizzas = pizzeriaController.getAllPizzas(page, size);
